...
import { Box, Flex, FormLabel, Image, useToast } from '@chakra-ui/react'
import InputComponent from '../../../components/authComponent/inputComponent'
import { useParams } from 'react-router-dom'
import ModalLayout from '../../../components/modalLayout'
import ViewAvatar from '../../../components/authComponent/viewAvatar'
...
function SetupUser() {
    ...
    const { adduserDetails, isLoading } = useAuth()
    ...
    const handleSubmit =(e: any)=> {
        ...
        if (!userName) { 
            toast({
                title: "Enter UserName To Continue",
                position: "bottom",
                status: "error",
                isClosable: true,
            }) 
            ...
            toast({
                title: "Pick an Avatar To Continue",
                position: "bottom",
                status: "error",
                isClosable: true,
            })
            ...
            <Box width={"full"} mt="30px" > 
                <form style={{width: "100%"}} onSubmit={(e)=> handleSubmit(e)}> 
                    <Box display={"flex"} flexDirection={"column"} gap={"20px"} >
                        <Flex onClick={()=> setOpen(true)} cursor={"pointer"} w={"99px"} mx={"auto"} rounded={"full"} h={"99px"} justifyContent={"center"} alignItems={"center"} bg={"#6670852B"} >
                            {avatar && (
                                <Image w={"full"} h={"full"} objectFit={"cover"} src={avatar} rounded={"full"} />
                            )}
                            {!avatar && (
                        ...
                        <Box width={"full"} > 
                            ...
                            <InputComponent onChange={(e: any)=> setUserName(e.target?.value)} type="text" placeholder="@michael" right={true} rightIcon={
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <Box width={"full"} display={"flex"} mt={"6px"} gap={"21px"} alignItems={"center"} >
                            <SubmitBtn name="Continue" loading={isLoading} />
                ...
                <ModalLayout open={open} size={["full", "full", "5xl"]} close={setOpen} title={""} hide={true} >
                    <ViewAvatar getdata={setAvatar} close={setOpen} />
                </ModalLayout>
            </Box>
        </AuthLayout>
    )
