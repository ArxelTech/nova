...
function FriendsTab(props: Props) {
    const {} = props 
    ...
    return (
        <Box  w={"full"} overflowY={"auto"} position={"relative"} display={"flex"} sx={{ flex: '1 0 auto' }} gap={"28px"} flexDirection={"column"}>
            {friend?.map((item: any) => {
                return( 
                    <Flex key={item?.name} alignItems={"center"} py={"14px"} px={"10px"} borderBottom={"0px"} justifyContent={"space-between"} >
            ...
            <Box p={"16px"} rounded={"8px"} bg={"brand.btnbg"} w={"full"} >
                <Text fontFamily={"700"} color={"brand.gray200"} >Need more friends? 😃</Text>
                <Text fontFamily={"400"} color={"brand.gray200"}>Seems like some other people are watching too? Want to make a new watch party friend. Add them as well</Text>
            </Box>
            <Text my={"14px"} fontFamily={"700"} color={"brand.gray200"} >Others watching right now</Text>
            {friend?.map((item: any) => {
                return( 
