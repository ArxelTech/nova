import { Box, Flex, Text } from '@chakra-ui/react'
import React from 'react' 
import WatchTabs from './watchtabs'

interface Props {
    max: any,
    show: boolean,
    change?: any
}

function WatchPartyOption(props: Props) {
    const {
        show,
        max,  
    } = props

    const [tab, setTab] = React.useState(-1)

    const clickHandler =(item: number)=> {
        if(item === tab){
            setTab(-1)
        } else {
            setTab(item)
        }
        max(false)
    }

    return (
        <Box width={"full"} position={"relative"} >
            <Text fontWeight={700} display={["none", "none", "flex"]} color={"brand.gray200"} >Watch Party Options</Text>
            <Flex mt={["0px", "0px","24px"]} justifyContent={[ "space-between", "space-between","space-around"]} w="full" >
                <Box display={["flex", "flex", "none"]} w={[ "auto", "auto" ,"full"]} onClick={()=> clickHandler(-1)}  cursor="pointer"  alignItems="center" flexDirection="column" > 
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === -1 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === -1 ? "brand.gray200": "brand.inactive"} >Chat</Text>
                </Box>
                <Box display={["flex", "flex", "none"]} w={[ "auto", "auto" ,"full"]} onClick={()=> clickHandler(-2)}  cursor="pointer"  alignItems="center" flexDirection="column" > 
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === -2 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === -2 ? "brand.gray200": "brand.inactive"} >Friends</Text>
                </Box>
                <Box display={["flex", "flex", "none"]} w={[ "auto", "auto" ,"full"]} onClick={()=> clickHandler(-3)}  cursor="pointer"  alignItems="center" flexDirection="column" > 
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === -3 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === -3 ? "brand.gray200": "brand.inactive"} >Option</Text>
                </Box>
                <Box display="flex" onClick={()=> clickHandler(0)} w={[ "auto", "auto" ,"full"]}  cursor="pointer" alignItems="center" flexDirection="column" > 
                    <Box pr="2px" cursor="pointer" w="55px" h="55px" bg={tab === 0 ? "brand.base" : "transparent"} border="1px" borderColor={tab === 0 ? "brand.base" : "#667085"} rounded="full" display={["none", "none", "flex"]} alignItems="center" justifyContent="center" >
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.09 13.51L15.92 17.49M15.91 6.51L9.09 10.49M21.5 5C21.5 6.65685 20.1569 8 18.5 8C16.8431 8 15.5 6.65685 15.5 5C15.5 3.34315 16.8431 2 18.5 2C20.1569 2 21.5 3.34315 21.5 5ZM9.5 12C9.5 13.6569 8.15685 15 6.5 15C4.84315 15 3.5 13.6569 3.5 12C3.5 10.3431 4.84315 9 6.5 9C8.15685 9 9.5 10.3431 9.5 12ZM21.5 19C21.5 20.6569 20.1569 22 18.5 22C16.8431 22 15.5 20.6569 15.5 19C15.5 17.3431 16.8431 16 18.5 16C20.1569 16 21.5 17.3431 21.5 19Z" stroke={tab === 0 ? "white": "#667085"} stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </Box>
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === 0 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === 0 ? "brand.gray200": "brand.inactive"} >Share</Text>
                </Box>
                <Box display={["none", "none", "flex"]} w={[ "auto", "auto" ,"full"]}onClick={()=> clickHandler(1)}  cursor="pointer" alignItems="center" flexDirection="column" > 
                    <Box pr="2px" cursor="pointer" w="55px" h="55px" bg={tab === 1 ? "brand.base" : "transparent"} border="1px" borderColor={tab === 1 ? "brand.base" : "#667085"} rounded="full" display={["none", "none", "flex"]} alignItems="center" justifyContent="center" >
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M22.5 12C22.5 6.48 18.02 2 12.5 2C6.98 2 2.5 6.48 2.5 12C2.5 17.52 6.98 22 12.5 22" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M8.49961 3H9.49961C7.54961 8.84 7.54961 15.16 9.49961 21H8.49961" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M15.5 3C16.47 5.92 16.96 8.96 16.96 12" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M3.5 16V15C6.42 15.97 9.46 16.46 12.5 16.46" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M3.5 9.0001C9.34 7.0501 15.66 7.0501 21.5 9.0001" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M18.7 21.4C20.4673 21.4 21.9 19.9673 21.9 18.2C21.9 16.4327 20.4673 15 18.7 15C16.9327 15 15.5 16.4327 15.5 18.2C15.5 19.9673 16.9327 21.4 18.7 21.4Z" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                            <path d="M22.5 22L21.5 21" stroke={tab === 1 ? "white": "#667085"} stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </Box>
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === 1 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === 1 ? "brand.gray200": "brand.inactive"} >Map</Text>
                </Box>
                <Box display="flex" onClick={()=> clickHandler(2)}  w={[ "auto", "auto" ,"full"]} cursor="pointer" alignItems="center" flexDirection="column" > 
                    <Box pr="2px" cursor="pointer" w="55px" h="55px" bg={tab === 2 ? "brand.base" : "transparent"} border="1px" borderColor={tab === 2 ? "brand.base" : "#667085"} rounded="full" display={["none", "none", "flex"]} alignItems="center" justifyContent="center" >
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_6_48349)">
                                <path d="M1.5 1.00009L23.5 23.0001M15.5 9.34009V4.00009C15.5007 3.25612 15.225 2.53841 14.7264 1.98629C14.2277 1.43417 13.5417 1.08703 12.8015 1.01227C12.0613 0.937497 11.3197 1.14043 10.7207 1.58168C10.1217 2.02293 9.70805 2.671 9.56 3.40009M17.5 16.9501C16.5238 17.9465 15.2721 18.6285 13.9056 18.9087C12.5391 19.1888 11.12 19.0543 9.8305 18.5224C8.54096 17.9904 7.43976 17.0854 6.66817 15.9232C5.89658 14.7611 5.48979 13.395 5.5 12.0001V10.0001M19.5 10.0001V12.0001C19.4996 12.4125 19.4628 12.8241 19.39 13.2301M12.5 19.0001V23.0001M8.5 23.0001H16.5M9.5 9.00009V12.0001C9.50052 12.5931 9.67675 13.1726 10.0064 13.6654C10.3361 14.1583 10.8045 14.5424 11.3523 14.7692C11.9002 14.9961 12.5029 15.0555 13.0845 14.94C13.6661 14.8244 14.2005 14.5391 14.62 14.1201L9.5 9.00009Z" stroke={tab === 2 ? "white": "#667085"} stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </g>
                            <defs>
                                <clipPath id="clip0_6_48349">
                                    <rect width="24" height="24" fill="white" transform="translate(0.5)"/>
                                </clipPath>
                            </defs>
                        </svg>
                    </Box>
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === 2 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === 2 ? "brand.gray200": "brand.inactive"} >Talk</Text>
                </Box>
                <Box display={["none", "none", "flex"]} w={[ "auto", "auto" ,"full"]}  onClick={()=> clickHandler(3)}  cursor="pointer" alignItems="center" flexDirection="column" > 
                    <Box pr="2px" cursor="pointer" w="55px" h="55px" bg={tab === 3 ? "brand.base" : "transparent"} border="1px" borderColor={tab === 3 ? "brand.base" : "#667085"} rounded="full" display={["none", "none", "flex"]} alignItems="center" justifyContent="center" >
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M22.5 11.0801V12.0001C22.4988 14.1565 21.8005 16.2548 20.5093 17.9819C19.2182 19.7091 17.4033 20.9726 15.3354 21.584C13.2674 22.1954 11.0573 22.122 9.03447 21.3747C7.01168 20.6274 5.28465 19.2462 4.11096 17.4372C2.93727 15.6281 2.37979 13.4882 2.52168 11.3364C2.66356 9.18467 3.49721 7.13643 4.89828 5.49718C6.29935 3.85793 8.19279 2.71549 10.2962 2.24025C12.3996 1.76502 14.6003 1.98245 16.57 2.86011M22.5 4.00011L12.5 14.0101L9.50001 11.0101" stroke={tab === 3 ? "white" : "#667085"} stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                    </Box>
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === 0 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === 3 ? "brand.gray200": "brand.inactive"} >Request</Text>
                </Box>
                <Box display={["none", "none", "flex"]} w={[ "auto", "auto" ,"full"]} onClick={()=> clickHandler(4)}  cursor="pointer" alignItems="center" flexDirection="column" > 
                    <Box pr="2px" cursor="pointer" w="55px" h="55px" bg={tab === 4 ? "brand.base" : "transparent"} border="1px" borderColor={tab === 4 ? "brand.base" : "#667085"} rounded="full" display={["none", "none", "flex"]} alignItems="center" justifyContent="center" >
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_6_48359)">
                                <path d="M12.5 15C14.1569 15 15.5 13.6569 15.5 12C15.5 10.3431 14.1569 9 12.5 9C10.8431 9 9.5 10.3431 9.5 12C9.5 13.6569 10.8431 15 12.5 15Z" stroke={tab === 4 ? "white" : "#667085"} stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                                <path d="M19.9 15C19.7669 15.3016 19.7272 15.6362 19.786 15.9606C19.8448 16.285 19.9995 16.5843 20.23 16.82L20.29 16.88C20.476 17.0657 20.6235 17.2863 20.7241 17.5291C20.8248 17.7719 20.8766 18.0322 20.8766 18.295C20.8766 18.5578 20.8248 18.8181 20.7241 19.0609C20.6235 19.3037 20.476 19.5243 20.29 19.71C20.1043 19.896 19.8837 20.0435 19.6409 20.1441C19.3981 20.2448 19.1378 20.2966 18.875 20.2966C18.6122 20.2966 18.3519 20.2448 18.1091 20.1441C17.8663 20.0435 17.6457 19.896 17.46 19.71L17.4 19.65C17.1643 19.4195 16.865 19.2648 16.5406 19.206C16.2162 19.1472 15.8816 19.1869 15.58 19.32C15.2842 19.4468 15.032 19.6572 14.8543 19.9255C14.6766 20.1938 14.5813 20.5082 14.58 20.83V21C14.58 21.5304 14.3693 22.0391 13.9942 22.4142C13.6191 22.7893 13.1104 23 12.58 23C12.0496 23 11.5409 22.7893 11.1658 22.4142C10.7907 22.0391 10.58 21.5304 10.58 21V20.91C10.5723 20.579 10.4651 20.258 10.2725 19.9887C10.0799 19.7194 9.81074 19.5143 9.5 19.4C9.19838 19.2669 8.86381 19.2272 8.53941 19.286C8.21502 19.3448 7.91568 19.4995 7.68 19.73L7.62 19.79C7.43425 19.976 7.21368 20.1235 6.97088 20.2241C6.72808 20.3248 6.46783 20.3766 6.205 20.3766C5.94217 20.3766 5.68192 20.3248 5.43912 20.2241C5.19632 20.1235 4.97575 19.976 4.79 19.79C4.60405 19.6043 4.45653 19.3837 4.35588 19.1409C4.25523 18.8981 4.20343 18.6378 4.20343 18.375C4.20343 18.1122 4.25523 17.8519 4.35588 17.6091C4.45653 17.3663 4.60405 17.1457 4.79 16.96L4.85 16.9C5.08054 16.6643 5.23519 16.365 5.294 16.0406C5.35282 15.7162 5.31312 15.3816 5.18 15.08C5.05324 14.7842 4.84276 14.532 4.57447 14.3543C4.30618 14.1766 3.99179 14.0813 3.67 14.08H3.5C2.96957 14.08 2.46086 13.8693 2.08579 13.4942C1.71071 13.1191 1.5 12.6104 1.5 12.08C1.5 11.5496 1.71071 11.0409 2.08579 10.6658C2.46086 10.2907 2.96957 10.08 3.5 10.08H3.59C3.92099 10.0723 4.242 9.96512 4.5113 9.77251C4.78059 9.5799 4.98572 9.31074 5.1 9C5.23312 8.69838 5.27282 8.36381 5.214 8.03941C5.15519 7.71502 5.00054 7.41568 4.77 7.18L4.71 7.12C4.52405 6.93425 4.37653 6.71368 4.27588 6.47088C4.17523 6.22808 4.12343 5.96783 4.12343 5.705C4.12343 5.44217 4.17523 5.18192 4.27588 4.93912C4.37653 4.69632 4.52405 4.47575 4.71 4.29C4.89575 4.10405 5.11632 3.95653 5.35912 3.85588C5.60192 3.75523 5.86217 3.70343 6.125 3.70343C6.38783 3.70343 6.64808 3.75523 6.89088 3.85588C7.13368 3.95653 7.35425 4.10405 7.54 4.29L7.6 4.35C7.83568 4.58054 8.13502 4.73519 8.45941 4.794C8.78381 4.85282 9.11838 4.81312 9.42 4.68H9.5C9.79577 4.55324 10.048 4.34276 10.2257 4.07447C10.4034 3.80618 10.4987 3.49179 10.5 3.17V3C10.5 2.46957 10.7107 1.96086 11.0858 1.58579C11.4609 1.21071 11.9696 1 12.5 1C13.0304 1 13.5391 1.21071 13.9142 1.58579C14.2893 1.96086 14.5 2.46957 14.5 3V3.09C14.5013 3.41179 14.5966 3.72618 14.7743 3.99447C14.952 4.26276 15.2042 4.47324 15.5 4.6C15.8016 4.73312 16.1362 4.77282 16.4606 4.714C16.785 4.65519 17.0843 4.50054 17.32 4.27L17.38 4.21C17.5657 4.02405 17.7863 3.87653 18.0291 3.77588C18.2719 3.67523 18.5322 3.62343 18.795 3.62343C19.0578 3.62343 19.3181 3.67523 19.5609 3.77588C19.8037 3.87653 20.0243 4.02405 20.21 4.21C20.396 4.39575 20.5435 4.61632 20.6441 4.85912C20.7448 5.10192 20.7966 5.36217 20.7966 5.625C20.7966 5.88783 20.7448 6.14808 20.6441 6.39088C20.5435 6.63368 20.396 6.85425 20.21 7.04L20.15 7.1C19.9195 7.33568 19.7648 7.63502 19.706 7.95941C19.6472 8.28381 19.6869 8.61838 19.82 8.92V9C19.9468 9.29577 20.1572 9.54802 20.4255 9.72569C20.6938 9.90337 21.0082 9.99872 21.33 10H21.5C22.0304 10 22.5391 10.2107 22.9142 10.5858C23.2893 10.9609 23.5 11.4696 23.5 12C23.5 12.5304 23.2893 13.0391 22.9142 13.4142C22.5391 13.7893 22.0304 14 21.5 14H21.41C21.0882 14.0013 20.7738 14.0966 20.5055 14.2743C20.2372 14.452 20.0268 14.7042 19.9 15Z" stroke={tab === 4 ? "white" : "#667085"} stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </g>
                            <defs>
                                <clipPath id="clip0_6_48359">
                                    <rect width="24" height="24" fill="white" transform="translate(0.5)"/>
                                </clipPath>
                            </defs>
                        </svg>
                    </Box>
                    <Text borderBottom={["4px" ,"4px", "0px"]} borderColor={tab === 4 ? "brand.base": "transparent"} pb={["10px", "10px", "0px"]} mt="8px" fontSize="14px" fontFamily="400" color={tab === 4 ? "brand.gray200": "brand.inactive"} >Settings</Text>
                </Box>
            </Flex> 
            <WatchTabs show={show} tab={tab} max={max} />
        </Box>
    )
}

export default WatchPartyOption  